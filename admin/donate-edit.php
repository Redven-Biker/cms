...
if (strlen($_SESSION['id']==0)) {
  header('location:assets/libs/logout.php');
    ...
    if(isset($_POST['Submit']))
    {
      ...
      $url=$_POST['url'];
      $id=intval($_GET['id']);
      $reponse = $bdd->query("UPDATE donate set title='$title', img='$img', description='$description', url='$url' where id='$id'");
    $_SESSION['msg']="Updated";
    }?>
<?php $reponse = $bdd->query("SELECT * from donate WHERE id='".$_GET['id']."'"); while ($data = $reponse->fetch()) { ?>
        <div id="page-wrapper" >
		  <div class="header"> 
    ...
    <form class="col s12" method="post" action="">
      ...
      <div class="row">
        <div class="input-field col s6">
          <h5>Url</h5>
          <input type="text" name="url" class="validate" value="<?php echo $data['url']; ?>">
        </div>
      </div>
